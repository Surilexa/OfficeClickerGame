...
function PasteTemplateData(temp_tiles, tileset_id, tileWidth, tileHeight, new_layer_name, offset_tiles_x, offset_tiles_y, debug_mode){
	...
	//var map_h = tilemap_get_height(tilemap_id);
	 for (var i = 0; i < array_length(temp_tiles); i++){
		 minx = min(minx, temp_tiles[i][1]);
		 maxx = max(maxx, temp_tiles[i][1]);
		 miny = min(miny, temp_tiles[i][2]);
		 maxy = max(maxy, temp_tiles[i][2]);
	 }
	 var map_w = maxx +1 - minx;
	 var map_h = maxy+1 - miny;
	 
	// show_debug_message(string(map_w));
	// show_debug_message(string(map_h));
	...
	//show_debug_message(string(tileset_id));
